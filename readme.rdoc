...
1. Comment out lines which starting with auth_basic in {/etc/nginx/sites-available/backuppc}[http://github.com/Zloy/backuppc_on_nginx/master/etc/nginx/sites-available/backuppc]. That disables authentication to simplify the first run.
...
I added  backuppc configs to {etc/backuppc/localhost.pl}[http://github.com/Zloy/backuppc_on_nginx/master/etc/backuppc/localhost.pl] to backup my settings. You may found useful to start defining your excluded files pool on the base of my one:
